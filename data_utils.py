...
def deal_quadruple(quadruple,  sentiment_dict):
    ...
    return f_quadruple_aspect, quadruple_sentiment, aspects, quadruples
...
class ACOSDataset(Dataset):
    def __init__(self, tokenizer, args, dataset_type):
        ...
        self.data_samples = self._build_examples(data_path, dataset_type)
    ...
    def _build_examples(self, data_path, dataset_type):
        """
        :param data_path: 数据存放路径
        :param dataset_type: 数据集类型(train、dev、test)
        ...
        for k in range(len(sentence_token)):
            ...
            f_quadruple_aspect,  quadruple_sentiment, aspects,  quadruples = deal_quadruple(quadruple,sentiment2id)
            forward_query_list = []
            forward_answer_list = []
            # backward_query_list = []
            ...
            for idx in range(len(f_quadruple_aspect)):
                ...
                sentiment_answer_list.append(quadruple_sentiment[idx])
            ...
            sample = DataSample(text, aspects,  quadruples,
    ...
    def _build_tokenized(self):
        ...
        for item in range(len(self.data_samples)):
            ...
            # Forward
            # aspect query
            temp_text = forward_querys[0]  + sentence_token
            ...
            for i in range(len(sentiment_querys)):
                # question_tokenized = category_querys[i] + ["null"] + sentence_token
                question_tokenized2 = sentiment_querys[i]  + sentence_token
                ...
                sentiment_category_lens.append(len(question_tokenized2))
